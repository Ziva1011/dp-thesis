...
#%%
    ...
    model = get_class('UNet3D', modules=[
        'pytorch3dunet.unet3d.model'
    ])
    model = model.to(device)

    formatter = DefaultTensorboardFormatter()
    optimizer= torch.optim.Adam(model.parameters(), lr=0.003, betas=(0.9, 0.999), weight_decay=0)
    unet = UNetTrainer(model=model, optimizer=optimizer, lr_scheduler=None, loss_criterion=loss_criterion,
                      eval_criterion=eval_criterion, loaders=loaders, tensorboard_formatter=formatter,
