...
class FPABlock(nn.Module):
    def __init__(self, in_channels, out_channels, upscale_mode="trilinear"):
        super(FPABlock, self).__init__()

        self.upscale_mode = upscale_mode
        if self.upscale_mode == "trilinear":
    ...
    def forward(self, x):
        ...
        b1 = F.interpolate(b1, size=(h, w, d), **upscale_parameters)
        ...
        x1 = self.conv1(x1)
        x = x + x1
        x = F.interpolate(x, size=(h, w, d), **upscale_parameters)
...
class GAUBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, upscale_mode: str = "trilinear"):
        super(GAUBlock, self).__init__()

        self.upscale_mode = upscale_mode
        self.align_corners = True if upscale_mode == "trilinear" else None
    ...
    def forward(self, x, y):
        ...
        """
        h, w, d = x.size(2), x.size(3), x.size(4)
        y_up = F.interpolate(y, size=(h, w, d), mode=self.upscale_mode, align_corners=self.align_corners)
        x = self.conv2(x)
        y = self.conv1(y)
        z = torch.mul(x, y)
        return y_up + z
...
class PANDecoder(nn.Module):
    def __init__(self, encoder_channels, decoder_channels, upscale_mode: str = "trilinear"):
        super().__init__()
